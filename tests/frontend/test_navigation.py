...
def main() -> None:
    ...
    health_css = FRONTEND / "health.css"
    ...
    assert "@media (min-width: 980px)" in health_css_source
    ...
    health_source = (FRONTEND / "health.html").read_text(encoding="utf-8")
    assert "health-action.css" not in health_source
    assert 'class="panel health-trends-details"' in health_source
